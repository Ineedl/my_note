...
void QRoundProgressBar::setUsedColor(const QColor color)
{
    m_usedColor = color;
}

void QRoundProgressBar::paintEvent(QPaintEvent *ev)
{
    Q_UNUSED(ev)
...
void QRoundProgressBar::drawBg(QPainter *painter)
{
...
void QRoundProgressBar::drawProcess(QPainter *painter)
{
    int width = this->width();
    int height = this->height();
    ...
    for(int i = startX; i < endX; i++) {
        ...
        //第二条波浪Y轴
        double waterY2 = (double)(A * qSin(w * i + m_offset + (endX / 2 * w))) + k;
    ...
    //第一条波浪
    path = totalPath.intersected(water1);       //totalPath和water1的交集   (园与sin值的交集)
    painter->setBrush(waterColor1);
    painter->drawPath(path);

    //第二条波浪挖去后的路径
    path = totalPath.intersected(water2);       //totalPath和water2的交集   (园与sin线段的交集)
...
void QRoundProgressBar::setMinValue(int value)
{
    m_minValue = value;
    update();
}

void QRoundProgressBar::setMaxValue(int value)
{
    m_maxValue = value;
    update();
}

void QRoundProgressBar::setValue(int v)
{
    m_value = v;
    update();
}

void QRoundProgressBar::setBorderWidth(int width)
{
    m_borderWidth = width;
    update();
}

void QRoundProgressBar::setWaterHeight(int height)
{
    m_waterHeight = height;
    update();
}

void QRoundProgressBar::start()
{
    if (m_timer) {
        m_timer->start();
    }
